      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 sm:mb-6">
        <div>
          <div className="flex items-center space-x-2">
            <RotateCcw className="w-5 h-5 text-indigo-400 flex-shrink-0" />
        ...
        <ResponsiveContainer width="100%" height="100%">
          {viewMode === 'days' ? (
            <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <ReferenceLine y={60} stroke="#3b82f6" strokeDasharray="3 3" label={{ value: 'DSO 60天', fill: '#60a5fa', fontSize: 9, position: 'right' }} />
              <ReferenceLine y={90} stroke="#f59e0b" strokeDasharray="3 3" label={{ value: 'DSI 90天', fill: '#fbbf24', fontSize: 9, position: 'right' }} />
              
              <Bar dataKey="dso" name="應收帳款天數 (DSO)" fill="#3b82f6" radius={[6, 6, 0, 0]} maxBarSize={32} />
              <Bar dataKey="dsi" name="存貨週轉天數 (DSI)" fill="#6366f1" radius={[6, 6, 0, 0]} maxBarSize={32} />
            ...
            <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="arTurnover" name="應收帳款週轉率" fill="#3b82f6" radius={[6, 6, 0, 0]} maxBarSize={32} />
              <Bar dataKey="inventoryTurnover" name="存貨週轉率" fill="#8b5cf6" radius={[6, 6, 0, 0]} maxBarSize={32} />
            </ComposedChart>
          ) : (
            <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
              ...
              <Bar dataKey="ccc" name="現金轉換循環 (CCC)" fill="#0ea5e9" radius={[6, 6, 0, 0]} maxBarSize={36} />
              <Line type="monotone" dataKey="ccc" name="CCC 趨勢走勢" stroke="#38bdf8" strokeWidth={3} dot={{ r: 4, fill: '#38bdf8' }} />
            </ComposedChart>
          )}
        </ResponsiveContainer>
      </div>
      ...
      {latest && (
        <div className="mt-4 sm:mt-5 pt-3 sm:pt-4 border-t border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3 text-xs">
          <div className="bg-slate-900/80 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border border-slate-800">
            <span className="text-slate-400 block text-[10px] sm:text-[11px] font-semibold uppercase tracking-wider">最新應收天數</span>
            <span className="text-sm sm:text-base font-bold text-blue-400 mt-1 block font-mono">{latest.ratios.dso} 天</span>
          ...
          <div className="bg-slate-900/80 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border border-slate-800">
            <span className="text-slate-400 block text-[10px] sm:text-[11px] font-semibold uppercase tracking-wider">最新存貨天數</span>
            <span className="text-sm sm:text-base font-bold text-indigo-400 mt-1 block font-mono">{latest.ratios.dsi} 天</span>
            <span className="text-[10px] sm:text-[11px] text-slate-500 block mt-0.5">
          ...
          <div className="bg-slate-900/80 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border border-slate-800">
            <span className="text-slate-400 block text-[10px] sm:text-[11px] font-semibold uppercase tracking-wider">現金轉換循環</span>
            <span className="text-sm sm:text-base font-bold text-cyan-400 mt-1 block font-mono">{latest.ratios.cashConversionCycle} 天</span>
            <span className="text-[10px] sm:text-[11px] text-slate-500 block mt-0.5">
