...
export const AiForecastChart: React.FC = () => {
  ...
  return (
    <div className="rounded-3xl border border-slate-800 bg-slate-900/60 backdrop-blur-sm p-6 shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <div className="flex items-center space-x-2">
            <div className="p-1.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-sm shadow-indigo-500/20">
        ...
        <div className="flex items-center bg-slate-950/70 p-1 rounded-2xl border border-slate-800 self-start sm:self-auto text-xs font-medium">
          <button
        ...
        <ResponsiveContainer width="100%" height="100%">
          {forecastMetric === 'revenue' ? (
            <ComposedChart data={series} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              {forecastStartLabel && (
                <ReferenceLine x={forecastStartLabel} stroke="#a855f7" strokeDasharray="3 3" label={{ value: '▶ AI 預測區間', fill: '#c084fc', fontSize: 10, position: 'insideTopLeft' }} />
              )}

              <Bar dataKey="revenueMillions" name="營業收入 (百萬)" fill="#3b82f6" radius={[6, 6, 0, 0]} maxBarSize={32} />
              <Area type="monotone" dataKey="netIncomeMillions" name="稅後淨利 (百萬)" stroke="#a855f7" strokeWidth={3} fill="url(#forecastArea)" dot={{ r: 5, fill: '#a855f7' }} />
            </ComposedChart>
          ) : forecastMetric === 'turnover' ? (
            <ComposedChart data={series} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="arTurnover" name="應收帳款週轉率 (次)" stroke="#38bdf8" strokeWidth={3} dot={{ r: 5, fill: '#38bdf8' }} />
              <Line type="monotone" dataKey="inventoryTurnover" name="存貨週轉率 (次)" stroke="#818cf8" strokeWidth={3} dot={{ r: 5, fill: '#818cf8' }} />
            </ComposedChart>
          ) : forecastMetric === 'margins' ? (
            <ComposedChart data={series} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
              ...
              <Line type="monotone" dataKey="grossMargin" name="毛利率 (Gross Margin %)" stroke="#10b981" strokeWidth={3} dot={{ r: 5, fill: '#10b981' }} />
            </ComposedChart>
          ) : (
            <ComposedChart data={series} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
              ...
              <Line type="monotone" dataKey="roe" name="股東權益報酬率 (ROE %)" stroke="#f59e0b" strokeWidth={3} dot={{ r: 5, fill: '#f59e0b' }} />
            </ComposedChart>
          )}
        </ResponsiveContainer>
      </div>
      ...
      <div className="mt-5 p-5 rounded-2xl bg-gradient-to-r from-indigo-950/40 via-slate-900 to-indigo-950/20 border border-indigo-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2 text-xs font-semibold text-indigo-300">
            ...
            <span className="bg-indigo-900/60 text-indigo-200 px-2.5 py-0.5 rounded-full border border-indigo-700/60 text-[10px] font-mono">
              預測信心度: {aiReport.forecast.confidenceLevel}%
            </span>
          </div>
        ...
        <div className="flex items-center space-x-4 text-xs bg-slate-950/80 p-3.5 rounded-2xl border border-slate-800 flex-shrink-0">
          <div>
            <span className="text-slate-400 block text-[11px]">預估營收成長率</span>
          ...
          <div>
            <span className="text-slate-400 block text-[11px]">預估目標 ROE</span>
