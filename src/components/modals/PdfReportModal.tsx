...
export const PdfReportModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md overflow-y-auto">
      <div className="bg-slate-900/95 border border-slate-800 rounded-3xl w-full max-w-4xl shadow-2xl overflow-hidden my-6 flex flex-col max-h-[92vh] backdrop-blur-md">
        ...
        <div className="px-6 py-5 bg-slate-950/80 border-b border-slate-800 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2.5">
            <button
              onClick={handlePrint}
              className="flex items-center space-x-1.5 px-3.5 py-2 rounded-2xl bg-slate-800/80 hover:bg-slate-700 text-slate-200 border border-slate-700/60 text-xs font-semibold transition"
            >
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-4.5 py-2 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition shadow-lg shadow-emerald-600/25 disabled:opacity-50"
            >
              <Download className={`w-4 h-4 ${isExporting ? 'animate-bounce' : ''}`} />
              <span>{isExporting ? '產生 PDF 中...' : '下載 PDF 報告'}</span>
            </button>
            <button
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-slate-950/60">
          ...
          <div
            ref={reportRef}
            id="printable-report"
            className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-3xl mx-auto text-slate-100 shadow-xl space-y-6"
          >
            ...
            <div className="border-b border-slate-800 pb-5">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <div className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 bg-blue-950/80 border border-blue-800/80 rounded-2xl text-blue-300 text-xs font-semibold">
                    <Sparkles className="w-3.5 h-3.5 text-amber-300" />
                    <span>AI 診斷評級: {health.rating} ({health.totalScore}分)</span>
                  </div>
                  <div className="text-[11px] text-slate-400 mt-2 flex items-center justify-end space-x-1">
            ...
            <div className="p-4.5 bg-slate-950/70 rounded-2xl border border-slate-800">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3 text-xs">
                ...
                <div className="p-3.5 bg-slate-950/60 rounded-2xl border border-slate-800">
                  <span className="text-slate-400 block text-[11px]">應收帳款週轉率 / DSO</span>
                  <div className="text-base font-bold text-blue-300 mt-0.5">
                    {curRatios.arTurnover} 次 <span className="text-xs text-slate-400 font-normal">({curRatios.dso} 天)</span>
                  </div>
                </div>

                <div className="p-3.5 bg-slate-950/60 rounded-2xl border border-slate-800">
                  <span className="text-slate-400 block text-[11px]">存貨週轉率 / DSI</span>
                  <div className="text-base font-bold text-indigo-300 mt-0.5">
                    {curRatios.inventoryTurnover} 次 <span className="text-xs text-slate-400 font-normal">({curRatios.dsi} 天)</span>
                  </div>
                </div>

                <div className="p-3.5 bg-slate-950/60 rounded-2xl border border-slate-800">
                  <span className="text-slate-400 block text-[11px]">現金轉換循環 (CCC)</span>
                ...
                <div className="p-3.5 bg-slate-950/60 rounded-2xl border border-slate-800">
                  <span className="text-slate-400 block text-[11px]">營業毛利率 (Gross Margin)</span>
                ...
                <div className="p-3.5 bg-slate-950/60 rounded-2xl border border-slate-800">
                  <span className="text-slate-400 block text-[11px]">營業利益率 (Operating Margin)</span>
                  <div className="text-base font-bold text-emerald-300 mt-0.5">
                ...
                <div className="p-3.5 bg-slate-950/60 rounded-2xl border border-slate-800">
                  <span className="text-slate-400 block text-[11px]">股東權益報酬率 (ROE)</span>
                  <div className="text-base font-bold text-amber-300 mt-0.5">
                    {curRatios.roe}% <span className="text-xs text-slate-400 font-normal">(EPS ${curRatios.eps})</span>
                  </div>
                </div>
            ...
            <div className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800 text-xs">
              ...
              <div className="flex items-center justify-between text-center font-mono">
                <div>
                  ...
                  <span className="text-emerald-400 font-bold text-sm">{curRatios.dupontNetMargin}%</span>
                </div>
                <span className="text-slate-500 font-sans">×</span>
                <div>
                  <span className="text-slate-400 block text-[10px]">總資產週轉率</span>
                  <span className="text-blue-400 font-bold text-sm">{curRatios.dupontAssetTurnover}次</span>
                </div>
                <span className="text-slate-500 font-sans">×</span>
                <div>
                  <span className="text-slate-400 block text-[10px]">權益乘數</span>
                  <span className="text-indigo-400 font-bold text-sm">{curRatios.dupontEquityMultiplier}倍</span>
                </div>
                <span className="text-slate-500 font-sans">=</span>
                <div>
                  <span className="text-amber-300 block text-[10px]">ROE 報酬率</span>
                  <span className="text-amber-300 font-bold text-sm">{curRatios.dupontRoe}%</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 text-xs">
                {aiReport.strategicRecommendations.map((rec, i) => (
                  <div key={i} className="p-3 bg-slate-950/60 rounded-2xl border border-slate-800 flex justify-between items-center">
                    <div>
                      <span className="font-semibold text-slate-200">【{rec.category}】</span>
                      <span className="text-slate-300 ml-1">{rec.action}</span>
                    </div>
                    <span className="text-[10px] text-indigo-300 flex-shrink-0 ml-2 font-mono">
