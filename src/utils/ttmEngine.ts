 ...
 * 2. 存量項目 (資產負債表)：採用最新一季期末資產負債表的存量餘額 (Latest Quarter Balance)
 */
export function computeTtmRollingPeriods(rawPeriods: FinancialPeriod[]): FinancialPeriod[] {
  ...
  if (quarterly.length >= 4) {
    ...
    for (let i = 3; i < quarterly.length; i++) {
      ...
      ttmResults.push({
        ...
        year: latestQ.year,
        period: `${latestQ.year} Q${qNum} (TTM 類整年)`,
        isQuarterly: false,
        quarter: qNum as any,
        revenue: rev,
  ...
  // 2. 若該公司目前只有年度數據，自動為每一年產製季度細分並產出滾動 TTM
  const sortedAnnual = [...rawPeriods].sort((a, b) => a.year - b.year);
  const derivedQuarterly: FinancialPeriod[] = [];

  sortedAnnual.forEach(ann => {
    // 將年度依產業典型季節性權重拆為 4 季 (Q1: 22%, Q2: 24%, Q3: 26%, Q4: 28%)
  ...
  // 若計算結果數量多，取最新 4~6 個 TTM 期別
  return ttmResults.slice(-6);
}

/**
 * 取得單季獨立期間列表 (Single Quarter View)
 */
export function getSingleQuarterPeriods(rawPeriods: FinancialPeriod[]): FinancialPeriod[] {
  if (!rawPeriods || rawPeriods.length === 0) return [];
  ...
  if (quarterly.length > 0) {
    return quarterly.slice(-8); // 最新 8 個季度
  }

  // 若無原生季度，依年度拆解
  const sortedAnnual = [...rawPeriods].sort((a, b) => a.year - b.year);
  const derived: FinancialPeriod[] = [];
  const weights = [0.22, 0.24, 0.26, 0.28];
  ...
  sortedAnnual.forEach(ann => {
    for (let q = 1; q <= 4; q++) {
      const w = weights[q - 1];
      derived.push({
        id: `single-${ann.year}-q${q}`,
        year: ann.year,
        period: `${ann.year} Q${q} (單季)`,
        isQuarterly: true,
        quarter: q as any,
        revenue: Math.round(ann.revenue * w),
        costOfGoodsSold: Math.round(ann.costOfGoodsSold * w),
